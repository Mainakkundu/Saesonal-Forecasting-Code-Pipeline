...
if __name__== '__main__':
    ...
    import sys
    start = time.time()
    h = pd.read_csv('train.txt')
    l = pd.read_csv('test.txt')

    print('=== Stationarity Tagging ===')
    resultF    = pd.DataFrame()
    terr = [v for v in h['trrtry'].unique()]
    for ter in terr:
        terrData   = h[h.trrtry==ter]
        ...
        for ind,option in enumerate(optionList):
            ...
            result   = pd.DataFrame()
            print('In progress' +'\n'+ter + '-----'+option )
            histData = h[(h.trrtry==ter)& (h.KEY==option)]
            if len(histData) > 52:
                t = St_tagger(histData,'QTY')
                result['ST_TGGR'] = t
                result['trrtry'] = ter
                result['KEY'] = option
            else:
    ...
    seasF    = pd.DataFrame()
    terr = [v for v in h1['trrtry'].unique()]
    for ter in terr:
            terrData   = h1[h1.trrtry==ter]
            optionList = list(set(terrData.KEY))
            ...
            for ind,option in enumerate(optionList):
                result   = pd.DataFrame()
                print('In progress' +'\n'+ter + '-----'+option )
                histData = h1[(h1.trrtry==ter)& (h1.KEY==option)]
                if len(histData) > 104:
    ...
    k = seasF.groupby(['trrtry','SUB_CLSS'])['SEASONAL_OVERLAP'].agg(pd.Series.max).reset_index()
    k['IS_SNL'] = np.where(k['SEASONAL_OVERLAP']>0,'NO','YES') ### this will help to run auto.arima 
    h1 = h1.merge(k,how='left',on=['trrtry','SUB_CLSS_NM'])
    print(h1.shape)
    h1.to_csv('seasonal_params.txt') ### this will consume by auto.arima code 
    
    print('== Seasonal Data selected ==')
    ...
    order_temp    = []
    new_h = h_snl.groupby(['trrtry','SUB_CLSS','date'])['QTY'].mean().reset_index() ## sub class level rollup
    terr = [v for v in new_h['trrtry'].unique()]
    for ter in terr:
        terrData   = new_h[new_h.trrtry==ter]
        clss_lst = list(set(terrData.SUB_CLSS))
        for i in clss_lst:
            print('In progress' +'\n'+ter + '-----'+str(i) )
            histData = new_h[(new_h.trrtry==ter)& (new_h.SUB_CLSS==i)]
            if len(histData) >= 104:
                sales = histData['QTY']
                sales = np.where(sales==0,1,sales)
                sales = np.log(sales)
    ...
    h_snl = h_snl.merge(orderdf,how='left',on=['trrtry','SUB_CLSS']) #### S_ORDER join 
    h_snl.shape

    print('==-Option Level Seasonal Simulation Starts for (p,d,q) ===')
    order_temp    = []
    terr = [v for v in h_snl['trrtry'].unique()]
    for ter in terr:
        terrData   = h_snl[h_snl.trrtry==ter]
        optionList = list(set(terrData.KEY))
        for i in optionList:
            print('In progress' +'\n'+ter + '-----'+str(i) )
            histData = h_snl[(h_snl.trrtry==ter)& (h_snl.KEY==i)]
            if len(histData) >= 104:
                sales = histData['QTY']
                sales = np.where(sales==0,1,sales)
                sales = np.log(sales)
                ...
                print('===SEASONAL SIMULATIONS TAGGING COMPLETE ===')
    orderdf=pd.DataFrame()
    orderdf['trrtry']=[f.split('_')[0] for f in order_temp]
    ...
    h_snl = h_snl.merge(orderdf,how='left',on=['trrtry','KEY'])
    ...
    terr = [v for v in h_snl['trrtry'].unique()]
    for ter in terr:
        histDataset1   = h_snl[h_snl.trrtry==ter]
        leadDataset    = l[l.trrtry==ter]
        optionList = list(set(terrData.KEY))
        print(len(optionList))
        ...
        for ind,option in enumerate(optionList):
            ...
            cols = ['trrtry','KEY','date','QTY','S_ARIMA_FCST']        
